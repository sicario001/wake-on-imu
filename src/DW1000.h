...
public:
	...
	*/
	...
	static void printChannelParams();
	...
	*/
	...
	static void myDefaults(); //Added
	static void setDefaults_chan2(); //Added
	static void setDefaults_fast();//Added for chan5 6.8Mbps
	...
	static constexpr byte MODE_UMPIRE_TRACKING[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024};
//private:
	...
	//Added Haige
	static void set_highpower();
	static void set_lowpower();
	static void set_specificpower(uint32_t pwr);
	
	/*Ashutosh Dhekne Modified - allow Rx to timeout*/
	static void setRxTimeout(uint16_t msTimeoutVal);
	static void setPACSize(byte newPAC);
	static void setSFDLength(byte sfdLength);
	...
	// Accumulator Memory Access
	static void getAccMem(byte acc_mem[], uint16_t start, uint16_t end);
