...
constexpr byte DW1000Class::MODE_UMPIRE_TRACKING[];
/*
...
void DW1000Class::select(uint8_t ss) {
	...
	writeValueToBytes(_networkAndAddress, 0xFA, LEN_PANADR);
...
void DW1000Class::begin(uint8_t irq, uint8_t rst) {
	...
	attachInterrupt(digitalPinToInterrupt(_irq), DW1000Class::handleInterrupt, CHANGE); // todo interrupt for ESP8266
}
...
void DW1000Class::tune() {
	...
	//TX power setting!!!!
	//set_lowpower();
	set_specificpower(0x001F1F00);
	Serial.println("Set specific power");
}
...
//Ashutosh Dhekne Addition
void DW1000Class::setRxTimeout(uint16_t msTimeoutVal) {	
	if (msTimeoutVal == 0)
	{
		interruptOnReceiveTimeout(false);
		writeSystemEventMaskRegister();
		setBit(_syscfg, LEN_SYS_CFG, RXWTOE_BIT, false);
		writeSystemConfigurationRegister();
	} else {
		interruptOnReceiveTimeout(true);
		writeSystemEventMaskRegister();
		uint16_t regVal = (uint16_t)(msTimeoutVal*1000);
		byte regValbytes[4];
		regValbytes[0] = (uint8_t) regVal;
		regValbytes[1] = (uint8_t) (regVal>>8);
		regValbytes[2] = 0;
		regValbytes[3] = 0;
		writeBytes(RX_FWTO, NO_SUB, regValbytes, LEN_RX_FWTO);
		setBit(_syscfg, LEN_SYS_CFG, RXWTOE_BIT, true);
		writeSystemConfigurationRegister();
	}
}

void DW1000Class::setPACSize(byte newPAC) {
	_pacSize = newPAC;
}

void DW1000Class::setSFDLength(byte sfdLength) {
...
void DW1000Class::printChannelParams()
{
	
	Serial.print("Channel: ");
	Serial.println(_channel);
	Serial.print("Ext frame? ");
	Serial.println(_extendedFrameLength);
	Serial.print("PAC: ");
	Serial.println(_pacSize);
	char msg[128];
	getPrintableDeviceMode(msg);
	Serial.println(msg);
}
...
void DW1000Class::getAccMem(uint8_t acc_mem[], uint16_t start, uint16_t end) {
	
	//Setup the correct clocks
	byte reg[2];
	readBytes(PMSC, PMSC_CTRL0_SUB, reg, 2);
	reg[0] = 0x48 | (reg[0] & 0xb3);
	reg[1] = 0x80 | reg[1];
	writeBytes(PMSC, PMSC_CTRL0_SUB, &reg[0], 1);
	writeBytes(PMSC, 0x01, &reg[1], 1);
	
	
	//byte curr_acc[500];
	readBytes(0x25, start*4, acc_mem, (end-start)*4);
	/*for (int i=0;i<40;i=i+4)
	{
		Serial.print(acc_mem[i+1], HEX);
		Serial.print(acc_mem[i], HEX);
		Serial.print(", ");
		Serial.print(acc_mem[i+3], HEX);
		Serial.print(acc_mem[i+2], HEX);
		Serial.println("");
	}*/
...
//Added by Ashutosh Dhekne, Haige Chen, Yifeng Cao
//Added to set high TX power
void DW1000Class::set_highpower(){
      //Set the higher transmit power always for all nodes

      byte txpower[LEN_TX_POWER];
      readBytes(TX_POWER, NO_SUB, txpower, LEN_TX_POWER);
	  uint32_t data;
	  data = txpower[0]<<24+txpower[1]<<16+txpower[2]<<8+txpower[3];
      data = (data & 0xFF0000FF) | (0x001F1F00) ; //1C = 14dB gain
	  
      writeValueToBytes(txpower, data, LEN_TX_POWER);
      writeBytes(TX_POWER, NO_SUB, txpower, LEN_TX_POWER);
}

//Added to set high TX power
void DW1000Class::set_lowpower(){
      //Set the higher transmit power always for all nodes
	  ...
	  uint32_t data;
	  data = txpower[0]<<24+txpower[1]<<16+txpower[2]<<8+txpower[3];
      data = (data & 0xFF0000FF) | (0x00C0C000) ; //C0 = 0dB gain//1C = 14dB gain
...
//Added to set high TX power
void DW1000Class::set_specificpower(uint32_t pwr){
      //Set the higher transmit power always for all nodes
	  ...
	  uint32_t data;
	  data = txpower[0]<<24+txpower[1]<<16+txpower[2]<<8+txpower[3];
      data = (data & 0xFF0000FF) | (pwr) ; //C0 = 0dB gain//1C = 14dB gain
	  
      writeValueToBytes(txpower, data, LEN_TX_POWER);
      writeBytes(TX_POWER, NO_SUB, txpower, LEN_TX_POWER);
}

//Working with TREK1000 New TDoA code
void DW1000Class::setDefaults_chan2() {
	if(_deviceMode == TX_MODE) {
		...
		*/
		...
		// default mode when powering up the chip
		// still explicitly selected for later tuning
		//const byte TREK1000_MODE[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024};
		//enableMode(TREK1000_MODE);
...
void DW1000Class::myDefaults() {
	...
	if(_deviceMode == TX_MODE) {
		...
		useExtendedFrameLength(false); //Changed from false
		useSmartPower(false);
		suppressFrameCheck(false); //Changed from false
		//for global frame filtering
		setFrameFilter(false);
		/* old defaults with active frame filter - better set filter in every script where you really need it
		setFrameFilter(true);
		...
		*/
		...
		// default mode when powering up the chip
		// still explicitly selected for later tuning
		const byte TREK1000_MODE[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_128};
		enableMode(TREK1000_MODE);
		...
		if(getPulseFrequency() == TX_PULSE_FREQ_16MHZ) {
			...
			setPreambleCode(PREAMBLE_CODE_64MHZ_);
		}
		*/
		Serial.print("PAC: ");
		Serial.println(_pacSize);
...
void DW1000Class::setDefaults_fast() {
    if(_deviceMode == TX_MODE) {
        ...
        */
        ...
        // default mode when powering up the chip
        // still explicitly selected for later tuning
        enableMode(MODE_LONGDATA_FAST_LOWPOWER);
